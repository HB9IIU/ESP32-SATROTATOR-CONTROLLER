...
void setup() {
    ...
    Serial.println("🔧 Main Script: AZIMUTH and ELEVATION Stepper Control");
    ...
    Serial.println("✅ AZI optocoupler sensor (GPIO19) configured as INPUT_PULLUP");
  
    pinMode(ELEoptocouplerSensor, INPUT_PULLUP);
    Serial.println("✅ ELE optocoupler sensor (GPIO23) configured as INPUT_PULLUP");
...
void loop()
{
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode == 200) {
      ...
      JsonDocument doc;  // ✅ use JsonDocument directly (no StaticJsonDocument)

      DeserializationError error = deserializeJson(doc, payload);
      if (!error) {

        if (doc["name"].is<const char*>() && doc["azimuth"].is<float>() && doc["elevation"].is<float>()) {
          const char* name = doc["name"];
          float azimuth = doc["azimuth"];
          float elevation = doc["elevation"];

          Serial.println("📡 Tracked Object:");
          ...
          moveAzimuthToAngle(azimuth);
          moveElevationToAngle(elevation);

        } else if (doc["status"].is<const char*>() && strcmp(doc["status"], "not_tracking") == 0) {
          Serial.println("🛑 Not tracking any object.");
        } else {
          Serial.println("⚠️  Unexpected JSON structure.");
        }
...
void connectToWiFi() {
  ...
  WiFiCredential networks[] = {
    {ssidPrimary, passwordPrimary},
    {ssidFallback, passwordFallback}
  };

  for (int attempt = 0; attempt < 2; attempt++) {
    const char* ssid = networks[attempt].ssid;
    const char* pass = networks[attempt].password;
...
int stepsFromAziAngle(float angle)
{
    const int steps_per_turn = 400;    // Stepper motor steps per full turn (e.g., 1.8°/step = 200 steps, 0.9°/step = 400 steps)
    const float gear_ratio_azi = 44.0; // Gear reduction ratio for AZIMUTH
    const float steps_per_degree = (steps_per_turn * gear_ratio_azi) / 360.0;
...
int stepsFromEleAngle(float angle)
{
    const int steps_per_turn = 400;    // Stepper motor steps per full turn
    const float gear_ratio_ele = 44.0; // Gear reduction ratio for ELEVATION
    const float steps_per_degree = (steps_per_turn * gear_ratio_ele) / 360.0;
    return angle * steps_per_degree;
}
